...
void print_usage(char *argv0)
{
  std::cerr << 
    ...
    "  -r|--rand                    Device emulation: generates random test result\n"
    "                               instead of probing phisical device\n"
    "  -h|--help\n"
    "  -v|--version\n"
    << std::endl;
}
...
int main(int argc, char *argv[])
{
  try
  {
    ...
    {
      int option_index = 0;
      int ch = getopt_long(argc, argv, "d:t:rhv", long_options, &option_index);
      if (ch == -1)
        break;
    ...
    if (!exit)
    {
      ...
      std::cout << dev_name << ": " << "enabled=" << enabled << std::endl;

      int result = rand() % 4;

      sqlite3cc::stmt stmt(db);
      const char *sql = "INSERT INTO tests (itd, dtm, result) VALUES (?1, ?2, ?3)";
      stmt.prepare(sql);
...
//        syslog << dev_name << " " << e.dtm() << " " << e.state() << std::endl;
      ...
      stmt.bind_int(2, dtm.sec());
      stmt.bind_int(3, result);
