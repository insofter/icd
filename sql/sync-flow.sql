...
UPDATE flow SET 
  ...
  work_time = (SELECT e.itd FROM externdb.flow e
    WHERE e.id == flow.id),
  test = (SELECT e.test FROM externdb.flow e
    WHERE e.id == flow.id)
  WHERE id IN (SELECT m.id FROM flow m JOIN externdb.flow e
    ...
    WHERE m.itd IS NOT e.itd OR m.dtm IS NOT e.dtm OR m.cnt IS NOT e.cnt
      ...
      OR m.test IS NOT e.test);

-- insert all records that are missing
INSERT INTO flow (id, itd, dtm, cnt, dark_time, work_time, test)
  SELECT e.id, e.itd, e.dtm, e.cnt, e.dark_time, e.work_time, e.test
